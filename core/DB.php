...
class DB
{
    ...
    {
        if (!$this->ketnoi) {
            $this->ketnoi = mysqli_connect('localhost', 'checkad5_test', 'checkad5_test', 'checkad5_test') or die('Bảo trì hệ thống định kỳ');
    ...
    {
        $this->connect();
       $row = $this->ketnoi->query("SELECT * FROM `usersda` WHERE `username` = '$username' ")->fetch_array();
        return $row;
    }
    function site($data)
    {
