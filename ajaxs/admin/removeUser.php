...
if (isset($_POST['id'])) {
    $getUser = $TN->get_row(" SELECT * FROM `usersda` WHERE `token`='" . check_string($_POST['token']) . "' ");
    if(!$getUser)
    {
        $data = json_encode([
            'status'    => 'error',
    ...
    $row = $TN->get_row("SELECT * FROM `usersda` WHERE `id` = '$id' ");
    if (!$row) {
        $data = json_encode([
            'status'    => 'error',
    ...
    $isRemove = $TN->remove("usersda", " `id` = '$id' ");
    if ($isRemove) {
        $TN->insert("logs", [
            'user_id'       => $getUser['id'],
            'ip'            => myip(),
